...
static Cond ReasonableROM(w32 *r)
{
	...
	if ((RL(&r[1]) < 0) || (RL(&r[1]) >= 49151) || ((short)RL(&r[1]) & 1) != 0)
		return false;
	for (i = 3; i < 10; i++)
		if (RL(&r[i]) < 0 || RL(&r[i]) >= 49151 || ((short)RL(&r[i]) & 1) != 0)
			return false;
	if (RL(&r[26]) < 0 || RL(&r[26]) >= 49151 || ((short)RL(&r[26]) & 1) != 0)
		return false;
	for (i = 32; i < 48; i++)
		if (RL(&r[i]) < 0 || RL(&r[i]) >= 49151 || ((short)RL(&r[i]) & 1) != 0)
			return false;
...
short LoadMainRom(void) /* load and modify QL ROM */
{
	...
	if (p && !QMD.no_patch)
	{
		...
		{
			int patch_addr = m68k_read_memory_32(0x0000004);

			m68k_write_memory_16_direct(patch_addr, FSTART_CMD_CODE);
		}
